...
# -*- coding: utf-8 -*-
"""
Script to calculate time-dependent Mg-Cl contact count
Function: Count Mg-Cl contacts (distance < 3.0Å) for each frame in the trajectory
...
output_file = 'mg_cl_contacts.dat'  # Output file

mg_selection = "name MG or element Mg"  # Magnesium ion selection statement
cl_selection = "name Cl- or element Cl"  # Chloride ion selection statement
CUTOFF = 3.0                     # Contact distance threshold (Å)
...
def main():
    ...
    mg_atoms = u.select_atoms(mg_selection)
    cl_atoms = u.select_atoms(cl_selection)
    
    print(f"System contains: {len(mg_atoms)} Mg²⁺ ions, {len(cl_atoms)} Cl⁻ ions")
